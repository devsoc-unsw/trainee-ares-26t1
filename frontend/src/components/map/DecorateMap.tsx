...
function isCellOccupiedOnLayer(layers: MapLayers, row: number, col: number, layerN: number): boolean {
...
export function DecorateMap({
  ...
  const canPlace = useCallback((row: number, col: number): boolean => {
    if (activeLayer == null) return false;
    if (cursor?.fromMap && cursor.fromMap.row === row && cursor.fromMap.col === col) return true;
  ...
  const handleMapClick = useCallback((e: React.MouseEvent) => {
        ...
        if (tileId != null) {
          ...
          onInventoryChange(
            inventory.some(i => i.tileId === tileId)
              ? inventory.map(i => i.tileId === tileId ? { ...i, count: i.count + 1 } : i)
    ...
    if (cursor) {
      ...
      const next = cloneLayers(layers);
      getLayer(next, cursor.tileId != null ? TILE_TYPES[cursor.tileId].layer : 0)[row][col] = cursor.tileId;
      onLayersChange(next);
      setCursor(null);
      return;
    }
    ...
    if (selectedInventoryId != null) {
      ...
      const remaining = nextInv.find(i => i.tileId === selectedInventoryId)?.count ?? 0;
  ...
  }, [cursor, selectedInventoryId, layers, inventory, canPlace, getCellFromEvent, onLayersChange, onInventoryChange]);
 
  // ── Inventory sidebar selection ──────────────────────────────────────────
  const handleInventorySelect = useCallback((tileId: number) => {
    // Drop any cursor item back to inventory first
    if (cursor) {
      if (cursor.fromMap) {
        // Restore to original position
        const next = cloneLayers(layers);
        getLayer(next, cursor.fromMap.layer)[cursor.fromMap.row][cursor.fromMap.col] = cursor.tileId;
        onLayersChange(next);
      } else {
        onInventoryChange(inventory.map(i =>
  ...
  const returnToInventory = useCallback((tileId: number) => {
    onInventoryChange(
      inventory.some(i => i.tileId === tileId)
        ? inventory.map(i => i.tileId === tileId ? { ...i, count: i.count + 1 } : i)
        : [...inventory, { tileId, count: 1 }]
  ...
  const cancelCursor = useCallback(() => {
    if (!cursor) { setSelectedInventoryId(null); return; }
    returnToInventory(cursor.tileId);
    setCursor(null);
  }, [cursor, returnToInventory]);
 
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => { if (e.key === "q") cancelCursor(); };
      ...
      <div className="flex-1 overflow-auto">
        ...
        >
          ...
          {Array.from({ length: rows }, (_, row) => (
            <div key={row} className="flex">
              {Array.from({ length: cols }, (_, col) => {
                ...
                const placeable  = isHovered && activeTileId != null && canPlace(row, col);
                const blocked    = isHovered && activeTileId != null && !canPlace(row, col);
                ...
                return (
                  ...
                  >
                    {/* Tiles */}
                    {tile0 && <img src={tile0.sprite} alt={tile0.name} draggable={false} className="absolute inset-0 block pointer-events-none" style={{ width: tileSize, height: tileSize, imageRendering: "pixelated", zIndex: 0 }} />}
                    {tile1 && <img src={tile1.sprite} alt={tile1.name} draggable={false} className="absolute inset-0 block pointer-events-none" style={{ width: tileSize, height: tileSize, imageRendering: "pixelated", zIndex: 1 }} />}
                    {tile2 && <img src={tile2.sprite} alt={tile2.name} draggable={false} className="absolute inset-0 block pointer-events-none" style={{ width: tileSize, height: tileSize, imageRendering: "pixelated", zIndex: 2 }} />}
 
                    {/* Available slot highlight */}
                    {activeTileId != null && activeLayer != null && !isCellOccupiedOnLayer(layers, row, col, activeLayer) && (
                      <div className="absolute inset-0 bg-green-700/20 pointer-events-none" style={{ zIndex: 5 }} />
                    )}
 
                    {/* Hover: placeable */}
                    {placeable && activeTile && (
                      <>
                        <div className="absolute inset-0 ring-2 ring-inset ring-lime-400 pointer-events-none" style={{ zIndex: 6 }} />
                        <img
                          src={activeTile.sprite}
                          alt={activeTile.name}
                          draggable={false}
                          className="absolute inset-0 block pointer-events-none opacity-70"
                          style={{ width: tileSize, height: tileSize, imageRendering: "pixelated", zIndex: 7 }}
                        />
                      </>
                    )}
                    ...
                    {/* Hover: blocked */}
                    {blocked && (
                      <div className="absolute inset-0 ring-2 ring-inset ring-red-600 bg-red-500/20 pointer-events-none" style={{ zIndex: 6 }} />
                    )}
                  </div>
                );
              })}
          ...
          {activeTile && hoverCell == null && (
            <div className="absolute top-0 left-0 pointer-events-none opacity-60" style={{ zIndex: 50 }}>
              <img src={activeTile.sprite} alt="" style={{ width: tileSize, height: tileSize, imageRendering: "pixelated" }} />
            </div>
          )}
        </div>
      </div>
