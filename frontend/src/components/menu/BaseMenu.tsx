    ...
    const menus: Record<MenuView, React.ReactNode> = {
        main: (
            <div className="grid grid-cols-3 gap-5 place-items-center">
                <MenuButton text="Settings" Icon={Settings} onClick={() => setMenuContent("settings")} />
                <MenuButton text="Shop" Icon={ShoppingBasket} onClick={() => setMenuContent("shop")} />
                <MenuButton text="Decorate" Icon={Paintbrush} onClick={() => setMenuContent("decorate")} />
                <MenuButton text="Add Task" Icon={CalendarPlus} onClick={() => setMenuContent("addTask")} />
                <MenuButton text="View Tasks" Icon={Calendar} onClick={() => setMenuContent("viewTask")} />
                <MenuButton text="Log out" Icon={LogOut} onClick={() => console.log("log out!")} />
            </div>
        ),
        ...
        <div className="ml-15">
            <WoodContainer>
                <div className="flex flex-col w-[25vw] h-[50vh] items-center p-2 gap-3">
                    <MenuHeader title={menuTitles[menuContent]} Icon={ArrowLeft} onBack={() => setMenuContent("main")} />
                    {menus[menuContent]}
                </div>
            </WoodContainer>
        </div>
